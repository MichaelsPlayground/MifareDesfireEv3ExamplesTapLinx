 ...
 */
public class MainActivityOrgV3 extends Activity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        boolean readPermission = (ContextCompat.checkSelfPermission(MainActivityOrgV3.this,
                Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED);

        if (!readPermission) {
            ActivityCompat.requestPermissions(MainActivityOrgV3.this,
 ...
 */
            ...
            if (bWriteAllowed) {
                ...
                desFireEV3.createApplication(appId, appsettings);
                desFireEV3.selectApplication(appId);

                desFireEV3.createFile(fileNo, new DESFireFile.StdDataFileSettings(
                        IDESFireEV3.CommunicationType.Plain, (byte) 0, (byte) 0, (byte) 0, (byte) 0, fileSize));

                desFireEV3.authenticate(0, IDESFireEV3.AuthType.Native, KeyType.TWO_KEY_THREEDES, objKEY_2KTDES);
                desFireEV3.writeData(0, 0, data);
                showMessage(
                        "Data Read from the card : "
                                + Utilities.dumpBytes(desFireEV3.readData(0, 0,
                                5)), 'd');
                showMessage(
                        "Free Memory of the Card : " + desFireEV3.getFreeMemory(),
                        'd');
                desFireEV3.getReader().close();
            }
    ...
    protected void showMessage(final String str, final char where) {
        ...
        switch (where) {
            ...
            case 't':
                Toast.makeText(MainActivityOrgV3.this, "\n" + str, Toast.LENGTH_SHORT)
                        .show();
                break;
            case 'l':
                NxpLogUtils.i(TAG, "\n" + str);
    ...
    private void showDisclaimer() {
        ...
        AlertDialog.Builder alert = new AlertDialog.Builder(MainActivityOrgV3.this);
        ...
        switch (requestCode) {
            case STORAGE_PERMISSION_WRITE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(MainActivityOrgV3.this, "Requested permission granted", Toast.LENGTH_LONG).show();

                } else {
                    Toast.makeText(MainActivityOrgV3.this, "The app was not allowed to write to your storage. Hence, it cannot function properly. Please consider granting it this permission", Toast.LENGTH_LONG).show();
                }
                break;
            }
        }
